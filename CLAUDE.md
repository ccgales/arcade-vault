...
- `/spec-impl-game` (`.claude/skills/spec-impl-game/SKILL.md`, repo-specific) — same implementation behavior as `/spec-impl` (defers to it for the phases themselves), but scoped to game specs, also resolving specs under `specs/game-jam/<game-id>/`. Once implemented and `npm run build` passes, it automatically chains `skin-designer` → `mobile-porter detalle`, strictly one after the other, never in parallel.
...
- **`skin-designer`** (subagent, `.claude/agents/skin-designer.md`) — audits and implements the `clasico`/`neon`/`retro` skins for the 4 real games, one game per run, always the one the user names (invoking it with no argument, several ids, or "all" runs a read-only audit instead — it never implements in batch or picks the game itself). Tracks progress in `references/games-with-themes.md`, measures WCAG contrast for every color against its skin's own dark background, and writes only `lib/skins.ts`, `components/SkinPicker.tsx`, the skins block of `app/globals.css`, `specs/NN-skins.md`, that tracking file, and the color-only expressions inside the current game's `components/games/<Name>.tsx`. Never touches gameplay, scoring, `REAL_GAMES`, or the CRT chrome itself.
- **`mobile-porter`** (subagent, `.claude/agents/mobile-porter.md`) — audits and ports the site to mobile, one zone per run, always the one the user names (`home`, `biblioteca`, `detalle`, `salon`, `acerca-de`, `auth`, `marco`; invoking it with no argument, several zones, or "all" runs a read-only audit instead). Measures in a real Chrome at 360/390/768px (overflow probes, 44px touch-target probes) rather than eyeballing it, and tracks progress in `references/mobile-porting.md`. Uses `specs/11-controles-tactiles-movil.md` as doctrine (canonical `720px` breakpoint, 44px targets, 16px inputs, never block zoom) but **does not implement it**: the player screen (`/juegos/[id]/jugar`), the virtual gamepad, and the `.av-player`/`.crt*`/`.player-hud`/`.modal*` selectors all belong to `/spec-impl 11`. Writes only its zone's selectors in `app/globals.css`, that zone's component, `specs/12-porte-movil-del-sitio.md`, and that tracking file.
...
- Styling is Tailwind utility classes, plus a neon/CRT arcade theme (`neon-cyan`, `.crt-screen`, etc.) defined in `globals.css`. The app is permanently dark — there is no light mode, no `dark:` variants, no `data-theme`/`prefers-color-scheme` toggle anywhere.
